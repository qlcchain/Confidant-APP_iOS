...
- (instancetype)initWithFlag:(NSUInteger)flag{    
    //新建一个文件夹去保存
    _flag = flag;
    NSString *logsDirectory = [CSFileLogger getLogsDir:flag];
...
+ (CSFileManagerDefault *)getFileManager:(NSString *)logsDir {
    CSFileManagerDefault *defaultLogFileManager = [[CSFileManagerDefault alloc] initWithLogsDirectory:logsDir fileName:CSLogger_Name];
    return defaultLogFileManager;
}
