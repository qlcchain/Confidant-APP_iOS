...
- (int) getFriendNumberWithFriend:(NSString *) friendAddress
{
    NSParameterAssert(friendAddress);
    OCTTox *tox = [self.dataSource managerGetTox];
    OCTToxFriendNumber friendNumber = [tox friendNumberWithPublicKey:friendAddress error:nil];
    return friendNumber;
}
#pragma mark -得到好友的连接状态
- (OCTToxConnectionStatus) getFriendConnectStatuWithFriendNumber:(OCTToxFriendNumber) friendNumber
{
   // NSParameterAssert(friendNumber);
    OCTTox *tox = [self.dataSource managerGetTox];
    return [tox friendConnectionStatusWithFriendNumber:friendNumber error:nil];
}

- (BOOL) friendIsExitWithFriend:(NSString *) friendAddress
{
    NSParameterAssert(friendAddress);
    OCTTox *tox = [self.dataSource managerGetTox];
    OCTToxFriendNumber friendNumber = [tox friendNumberWithPublicKey:friendAddress error:nil];
    ...
    AppD.currentRouterNumber = friendNumber;
    return YES;
   // return [tox friendExistsWithFriendNumber:friendNumber];
}
...
- (BOOL)sendFriendRequestToAddress:(NSString *)address message:(NSString *)message error:(NSError **)error
{
    ...
    OCTToxFriendNumber friendNumber = [tox addFriendWithAddress:address message:@"123" error:error];
...
{
    [self.dataSource managerSaveTox];
    NSLog(@"----%d-------%d",friendNumber,AppD.currentRouterNumber);
     if (friendNumber == AppD.currentRouterNumber && status == OCTToxConnectionStatusNone) {
         AppD.isConnect = NO;
     }
