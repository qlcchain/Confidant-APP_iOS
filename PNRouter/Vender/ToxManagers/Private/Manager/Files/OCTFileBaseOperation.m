...
{
    NSLog(@"start loading file with identifier %@", self.operationId);
}
...
- (void)updateProgressIfNeeded:(OCTToxFileSize)bytesDone
{
    ...
    NSLog(@"progress %.2f, bytes per second %lld, eta %.0f seconds", self.progress, self.bytesPerSecond, self.eta);
