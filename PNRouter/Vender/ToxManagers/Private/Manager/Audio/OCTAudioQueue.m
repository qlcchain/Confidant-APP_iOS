...
- (BOOL)begin:(NSError **)error
{
    ...
    if (! self.audioQueue) {
        OSStatus res = [self createAudioQueue];
        if (res != 0) {
            NSLog(@"Can't create the audio queue again after a presumably failed updateSampleRate:... call.");
            if (error) {
                *error = OCTErrorFromCoreAudioCode(res);
            }
            return NO;
...
- (BOOL)setDeviceID:(NSString *)deviceID error:(NSError **)err
{
    ...
    if (ok != 0) {
        NSLog(@"setDeviceID: Error while live setting device to '%@': %d", deviceID, ok);
        if (err) {
            *err = OCTErrorFromCoreAudioCode(ok);
        }
    }
...
{
    NSLog(@"updateSampleRate %u, %u", sampleRate, (unsigned int)numberOfChannels);
    ...
    OSStatus res = [self createAudioQueue];
    if (res != 0) {
        NSLog(@"oops, could not recreate the audio queue: %d after samplerate/nc change. enjoy your overflowing buffer", (int)res);
        if (err) {
