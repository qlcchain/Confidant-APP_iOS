...
- (void)stop
{
    ...
    @synchronized(self) {
        if (! self.timer) {
            NSLog(@"toxav isn't running, nothing to stop");
            return;
        }
...
- (void)dealloc
{
    ...
    NSLog(@"dealloc called, toxav killed");
}
...
- (BOOL)setAudioBitRate:(OCTToxAVAudioBitRate)bitRate force:(BOOL)force forFriend:(OCTToxFriendNumber)friendNumber error:(NSError **)error
{
    ...
    NSLog(@"setAudioBitRate:%lu, force:%d, friend:%d", (long)bitRate, force, friendNumber);
                          ...
                          void *userData)
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        NSLog(@"callIncomingCallback from friend %lu with audio:%d with video:%d", toxAV, (unsigned long)friendNumber, audioEnabled, videoEnabled);
                       ...
                       void *userData)
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSLog(@"callStateCallback from friend %d with state: %d", toxAV, friendNumber, cState);
                                ...
                                void *userData)
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        NSLog(@"audioBitRateStatusCallback from friend %d bitRate: %d", toxAV, friendNumber, bit_rate);
                                ...
                                void *userData)
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        NSLog(@"videoBitRateStatusCallback from friend %d bitRate: %d", toxAV, friendNumber, bit_rate);
