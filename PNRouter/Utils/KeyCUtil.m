...
// 写入router
+ (BOOL) saveRouterTokeychainWithValue:(NSDictionary *)value key:(NSString *) key
{
...
+ (NSArray *) getRouterWithKey:(NSString *) key
{
     NSString *modeJosn = [KeychainUtil getKeyValueWithKeyName:key];
    if (modeJosn) {
...
+ (BOOL) saveRouterTokeychainWithArr:(NSArray *)arr key:(NSString *)key {
